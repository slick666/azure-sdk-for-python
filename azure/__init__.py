...
def _fill_data_to_return_object(node, return_obj):
    ...
    for name, value in members.iteritems():
        if isinstance(value, _list_of):
            setattr(return_obj, name,
                    _fill_list_of(node, value.list_type, value.xml_element_name))
        elif isinstance(value, _scalar_list_of):
            setattr(return_obj, name, _fill_scalar_list_of(
                node, value.list_type, _get_serialization_name(name), value.xml_element_name))
        elif isinstance(value, _dict_of):
            setattr(
                return_obj, name, _fill_dict_of(
                    node, _get_serialization_name(name),
                    value.pair_xml_element_name, value.key_xml_element_name, value.value_xml_element_name))
...
def _general_error_handler(http_error):
    ...
    if http_error.status == 409:
        ...
        if http_error.respbody is not None:
            raise WindowsAzureError(_ERROR_UNKNOWN %
                                    http_error.message + '\n' + http_error.respbody)
        else:
