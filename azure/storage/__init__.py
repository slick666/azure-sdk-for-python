...
def _update_storage_blob_header(request, account_name, account_key):
    ...
    request.headers.append(
        ('Authorization', _sign_storage_blob_request(request, account_name, account_key)))
    ...
    for name, value in source.iteritems():
        ...
        if conv is None:
            raise WindowsAzureError(
                _ERROR_CANNOT_SERIALIZE_VALUE_TO_ENTITY % type(value).__name__)
