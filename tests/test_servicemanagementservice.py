...
class ServiceManagementServiceTest(AzureTestCase):
    ...
    def tearDown(self):
        if self.data_disk_info is not None:
            try:
                disk = self.sms.get_data_disk(self.data_disk_info[0], self.data_disk_info[
                                              1], self.data_disk_info[2], self.data_disk_info[3])
                try:
                    result = self.sms.delete_data_disk(
                        self.data_disk_info[0], self.data_disk_info[1], self.data_disk_info[2], self.data_disk_info[3])
                ...
                try:
                    for role in deployment.role_list:
                        role_props = self.sms.get_role(
                            self.hosted_service_name, deployment.name, role.role_name)
    ...
    def _create_hosted_service(self, name, location=None, affinity_group=None):
        ...
        result = self.sms.create_hosted_service(
            name, name + 'label', name + 'description', location, affinity_group, {'ext1': 'val1', 'ext2': 42})
    ...
    def _upload_chunks(self, file_path, blob_name, chunk_size):
        ...
        with open(file_path, 'rb') as f:
            while True:
                ...
                if data:
                    length = len(data)
                    self.bc.put_page(self.container_name, blob_name, data,
                                     'bytes=' + str(index) + '-' + str(index + length - 1), 'update')
                    index += length
                else:
                    break
    ...
    def _add_deployment(self, service_name, deployment_name, deployment_slot='Production'):
        ...
        result = self.sms.create_deployment(
            ...
            deployment_name + 'label', configuration, False, False, {'dep1': 'val1', 'dep2': 'val2'})
        self._wait_for_async(result.request_id)
    ...
    def _add_data_disk_from_blob_url(self, service_name, deployment_name, role_name, lun, label):
        ...
        result = self.sms.add_data_disk(
            service_name, deployment_name, role_name, lun, None, None, label, None, None, url)
        self._wait_for_async(result.request_id)
    ...
    def _create_vm_linux(self, service_name, deployment_name, role_name, target_container_name, target_blob_name):
        ...
        media_link = 'http://' + \
            credentials.getStorageServicesName(
            ) + '.blob.core.windows.net/' + target_container_name + '/' + target_blob_name
        ...
        self._create_service_certificate(
            service_name, SERVICE_CERT_DATA, SERVICE_CERT_FORMAT, SERVICE_CERT_PASSWORD)

        result = self.sms.create_virtual_machine_deployment(
            service_name, deployment_name, 'staging', deployment_name + 'label', role_name, system, os_hd, network_config=network, role_size='Small')
        self._wait_for_async(result.request_id)
    ...
    def test_create_hosted_service(self):
        ...
        # Act
        result = self.sms.create_hosted_service(
            self.hosted_service_name, label, description, location, None, {'ext1': 'val1', 'ext2': 'val2'})
    ...
    def test_update_hosted_service(self):
        ...
        result = self.sms.update_hosted_service(
            self.hosted_service_name, label, description, {'ext1': 'val1update', 'ext2': 'val2update', 'ext3': 'brandnew'})
        ...
        self.assertEqual(
            props.hosted_service_properties.extended_properties['ext2'], 'val2update')
        self.assertEqual(
            props.hosted_service_properties.extended_properties['ext3'], 'brandnew')
    ...
    def test_create_deployment(self):
        ...
        # Act
        result = self.sms.create_deployment(
            self.hosted_service_name, 'production', 'WindowsAzure1', package_url, 'deploylabel', configuration)
        self._wait_for_async(result.request_id)
    ...
    def test_swap_deployment(self):
        ...
        result = self.sms.swap_deployment(
            self.hosted_service_name, production_deployment_name, staging_deployment_name)
        self._wait_for_async(result.request_id)
    ...
    def test_upgrade_deployment(self):
        ...
        result = self.sms.upgrade_deployment(
            self.hosted_service_name, deployment_name, 'Auto', package_url, configuration, 'upgraded', True)
        self._wait_for_async(result.request_id)
    ...
    def test_walk_upgrade_domain(self):
        ...
        configuration = base64.b64encode(DEPLOYMENT_UPDATE_CONFIG)
        result = self.sms.upgrade_deployment(
            self.hosted_service_name, deployment_name, 'Manual', package_url, configuration, 'upgraded', True)
        self._wait_for_async(result.request_id)
    ...
    def test_rollback_update_or_upgrade(self):
        ...
        self.sms.upgrade_deployment(self.hosted_service_name, deployment_name,
                                    'Auto', package_url, configuration, 'upgraded', True)
    ...
    def test_reboot_role_instance(self):
        ...
        self._wait_for_role_instance_status(
            self.hosted_service_name, deployment_name, role_instance_name, 'ReadyRole')
    ...
    def test_get_service_certificate(self):
        ...
        result = self.sms.get_service_certificate(
            self.hosted_service_name, SERVICE_CERT_THUMBALGO, SERVICE_CERT_THUMBPRINT)
    ...
    def test_create_virtual_machine_deployment_linux(self):
        ...
        media_link = 'http://' + \
            credentials.getStorageServicesName(
            ) + '.blob.core.windows.net/' + self.container_name + '/' + role_name + '.vhd'
        pk = PublicKey(SERVICE_CERT_THUMBPRINT,
                       u'/home/unittest/.ssh/authorized_keys')
        pair = KeyPair(SERVICE_CERT_THUMBPRINT, u'/home/unittest/.ssh/id_rsa')
    ...
    def test_create_virtual_machine_deployment_windows_virtual_network(self):
        ...
        result = self.sms.create_virtual_machine_deployment(
            service_name, deployment_name, 'staging', deployment_name +
            'label',
            role_name, system, os_hd, network_config=network, role_size='Small', virtual_network_name=virtual_network_name)
        self._wait_for_async(result.request_id)
        self._wait_for_deployment_status(
            service_name, deployment_name, 'Running')
    ...
    def test_add_role_linux(self):
        ...
        media_link = 'http://' + \
            credentials.getStorageServicesName(
            ) + '.blob.core.windows.net/' + self.container_name + '/' + role_name2 + '.vhd'
    ...
    def test_update_role(self):
        ...
        self._create_vm_windows(
            service_name, deployment_name, role_name, 'vhds', self.hosted_service_name)
        self._wait_for_role_instance_status(
        ...
        result = self.sms.update_role(
            service_name, deployment_name, role_name, network_config=network, role_size='Medium')
        self._wait_for_async(result.request_id)
    ...
    def test_capture_role(self):
        ...
        result = self.sms.capture_role(
            service_name, deployment_name, role_name, 'Delete', image_name, image_label)
        self._wait_for_async(result.request_id)
    ...
    def test_update_os_image(self):
        ...
        result = self.sms.update_os_image(
            self.os_image_name, 'newlabel', LINUX_OS_VHD_URL, self.os_image_name, 'Linux')
        self._wait_for_async(result.request_id)
    ...
    def test_add_data_disk_from_disk_name(self):
        ...
        # Act
        result = self.sms.add_data_disk(
            service_name, deployment_name, role_name, lun, None, None, 'testdisklabel', self.disk_name)
        self._wait_for_async(result.request_id)

        # Assert
        self.assertTrue(
            self._data_disk_exists(service_name, deployment_name, role_name, lun))
    ...
    def test_update_data_disk(self):
        ...
        result = self.sms.update_data_disk(
            service_name, deployment_name, role_name, lun, None, None, updated_lun)
        self._wait_for_async(result.request_id)
        self.data_disk_info = (
        ...
        self.assertFalse(
            self._data_disk_exists(service_name, deployment_name, role_name, lun))
        self.assertTrue(
            self._data_disk_exists(service_name, deployment_name, role_name, updated_lun))
    ...
    def test_update_disk(self):
        ...
        result = self.sms.update_disk(
            self.disk_name, False, 'ptvslabelupdate', urlupdate, self.disk_name, 'Windows')
