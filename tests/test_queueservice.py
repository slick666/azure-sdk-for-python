...
class QueueServiceTest(AzureTestCase):
    ...
    def setUp(self):
        ...
        for i in range(10):
            self.test_queues.append(
                TEST_QUEUE_PREFIX + str(i) + getUniqueNameBasedOnCurrentTime(queue_base_name))
        for i in range(4):
            self.creatable_queues.append(
                'mycreatablequeue' + str(i) + getUniqueNameBasedOnCurrentTime(queue_base_name))
    ...
    def test_create_queue_fail_on_exist(self):
        # Action
        created = self.queue_client.create_queue(
            self.creatable_queues[0], None, True)
    ...
    def test_create_queue_with_options(self):
        # Action
        self.queue_client.create_queue(
            self.creatable_queues[1], x_ms_meta_name_values={'foo': 'test', 'bar': 'blah'})
    ...
    def test_delete_queue_fail_not_exist_already_exist(self):
        ...
        deleted = self.queue_client.delete_queue(
            self.creatable_queues[0], True)
    ...
    def test_list_queues_with_options(self):
        # Action
        queues_1 = self.queue_client.list_queues(
            prefix=TEST_QUEUE_PREFIX, maxresults=3)
        queues_2 = self.queue_client.list_queues(
            prefix=TEST_QUEUE_PREFIX, marker=queues_1.next_marker, include='metadata')
    ...
    def test_set_queue_metadata(self):
        ...
        self.queue_client.set_queue_metadata(
            self.creatable_queues[2], x_ms_meta_name_values={'foo': 'test', 'bar': 'blah'})
    ...
    def test_get_messages_with_options(self):
        ...
        result = self.queue_client.get_messages(
    ...
    def test_peek_messages_with_options(self):
        ...
        result = self.queue_client.peek_messages(
            self.test_queues[4], numofmessages=4)
    ...
    def test_delete_message(self):
        ...
        result2 = self.queue_client.get_messages(
            self.test_queues[6], numofmessages=32)
    ...
    def test_update_message(self):
        ...
        self.queue_client.update_message(self.test_queues[7], list_result1[
                                         0].message_id, 'new text', list_result1[0].pop_receipt, visibilitytimeout=0)
    ...
    def test_unicode_update_message_unicode_data(self):
        ...
        list_result1 = self.queue_client.get_messages(self.test_queues[7])
        self.queue_client.update_message(self.test_queues[7], list_result1[
                                         0].message_id, u'啊齄丂狛狜', list_result1[0].pop_receipt, visibilitytimeout=0)
        list_result2 = self.queue_client.get_messages(self.test_queues[7])
